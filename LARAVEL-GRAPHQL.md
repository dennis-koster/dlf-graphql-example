...
Met deze krachtige query-taal kunnen onze frontend ontwikkelaars exact de data opvragen die zij nodig hebben. Zo wordt er
geen onnodige data over de lijn gestuurd en kunnen we efficient nieuwe componenten ontwikkelen, zonder aanpassingen
aan de API. Daarbij biedt [GraphQL](https://graphql.org/) een belangrijke stap op het gebied van standaardisatie. Zo is
de communicatie tussen de backend en de frontend voorspelbaar geworden en kunnen we ons focussen op het ontwikkelen van
de wensen van de klant.

Met [Lighthouse](https://lighthouse-php.com/) is het opzetten van een GraphQL API in Laravel erg eenvoudig!
...
bij de installatie gaan genereren.
...
Tot slot installeren we een interactieve GraphQL Playground, die we gaan gebruiken om API calls te maken. Deze is standaard 
te bereiken op `http://<APP_URL>/graphiql`.
...
We zijn klaar om onze eerste API call maken! Onze basisinstallatie van Laravel en Lighthouse komt, out of the box, met 
twee queries om gebruikers op te halen. Deze vind je in `graphql/schema.graphql`.
...
resultaten gepagineerd worden teruggegeven. Met de `paginatorInfo` kunnen we zien hoeveel resultaten er in totaal
zijn en hoeveel pagina's er zijn. Vervang je deze directive met de [`@all`](https://lighthouse-php.com/6/api-reference/directives.html#all)
directive, dan krijg je alle resultaten terug.

### Meerdere queries in één request

Een van de grootste voordelen van GraphQL is het bundelen [van meerdere queries](https://graphql.org/#single-request). 
Met één request naar de API kan daarmee alle data opgevraagd worden die nodig is.
...
We weten nu hoe we data ophalen, maar hoe bewerken we data via de API? In GraphQL doen we dat 
met [mutations](https://graphql.org/learn/queries/#mutations). We gaan een mutation maken waarmee we een nieuwe 
gebruiker kunnen maken.
...
Met de standaard directives komen we een heel eind, zonder ook maar een regel PHP-code te schrijven. 
Natuurlijk dekken deze lang niet alle denkbare scenarios en kun je ook jouw eigen logica schrijven.

### Maatwerk mutation
Als voorbeeld nemen we een beheerpaneel voor admins. Via dit paneel moet het mogelijk zijn om het wachtwoord van een 
gebruiker te resetten. We willen zelf het nieuwe wachtwoord kunnen specificeren, maar als deze niet wordt meegegeven 
willen we dat de API een willekeurig wachtwoord genereert.
...
Met de `@field` directive verwijzen we naar de PHP class die verantwoordelijk is voor het afhandelen van deze 
mutation. Deze class bestaat nog niet, dus laten we die creëren.
...
```
### Queries uitbreiden
Soms doen de standaard directives **bijna** wat je wil, maar wil je de uitgevoerde 
...
wanneer je gebruik wilt maken van [Laravel Passport](https://laravel.com/docs/11.x/passport). 
Dankzij [daniel-de-wit/lighthouse-sanctum](https://github.com/daniel-de-wit/lighthouse-sanctum)
kun je snel aan de slag met met [Laravel Sanctum](https://laravel.com/docs/11.x/sanctum#main-content).

### De diepte in
Een GraphQL API opzetten is met Lighthouse een fluitje van een cent. De behandelde scenarios geven je hopelijk een goede 
basis om mee te starten, maar Lighthouse is erg uitgebreid. De [documentatie](https://lighthouse-php.com/6/getting-started/installation.html) 
